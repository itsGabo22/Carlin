...
import { WholesalePulseButton } from './WholesalePulseButton';
...
export function HeroSection({ slides = [] }: { slides?: any[] }) {
  ...
  const hasSlides = slides.length > 0 && !mediaError;
  const activeSlide = hasSlides ? slides[currentSlide] : undefined;
  ...
  return (
    <section
      className="relative min-h-screen overflow-hidden bg-brand-pink-dark group"
    ...
    >
      {/* LAYER 1 — Background (changes based on slides/config) */}
          ...
          <AnimatePresence initial={false}>
            ...
            >
              {activeSlide.type === 'VIDEO' ? (
                ...
                >
                  <source src={activeSlide.desktopUrl} />
                </video>
              ) : (
                <picture>
                  {activeSlide.mobileUrl && (
                    <source media="(max-width: 768px)" srcSet={activeSlide.mobileUrl} />
                  )}
      ...
      {/* Floating Decorations — sits above the overlay so shapes stay undimmed, as before */}
          ...
          >
            <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white leading-tight mb-6 tracking-tight drop-shadow-lg">
              {activeSlide?.title ? (
                ...
                <>Belleza que inspira, <br className="hidden sm:block" /><span className="text-brand-pink-light italic">precios que enamoran.</span></>
              )}
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
              ...
              {/* MAYORISTA BUTTON — static, always here, with pulse */}
              <WholesalePulseButton />
            </div>
          </m.div>
        </AnimatePresence>
      </div>
