...
export default async function CatalogoCategoriaPage({ params, searchParams }: PageParams & SearchParams) {
  const { path } = await params;
  const sp = await searchParams;

  // ── Resolución de la ruta completa ──────────────────────────────
  // Cada segmento debe encajar en la cadena real raíz → … → destino.
  const cadena = await resolveCategoryChain(path ?? []);
  if (!cadena) notFound();

  const actual = cadena[cadena.length - 1];

  const todas = await getActiveCategories();
  ...
  // Rango de precios real de la categoría, para acotar el filtro.
  const rango = await prisma.product.aggregate({
    where: { active: true, categoryId: { in: idsCategoria } },
  ...
  const subcategorias = childrenOf(todas, actual.id).map((c) => ({
    id: c.id,
    name: c.name,
    href: buildCategoryHref(c, porId),
    imageUrl: c.imageUrl,
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
      ...
      {/* Migas de pan: muestran la cadena completa, no solo 2 niveles */}
      ...
      <header className="mb-8 text-center">
        <h1 className="font-serif text-3xl font-bold text-brand-neutral-dark sm:text-4xl">{actual.name}</h1>
        {actual.description && (
          <p className="mx-auto mt-2 max-w-2xl text-brand-text">{actual.description}</p>
        )}
      </header>
      ...
      <div className="flex flex-col gap-8 md:flex-row">
        ...
        <aside className="w-full shrink-0 md:w-64">
          {/* En móvil el sidebar va plegado dentro de un <details> para no
              empujar la grilla; en escritorio queda siempre abierto y pegajoso. */}
