  ...
  try {
    ...
    if (type === 'IMAGE') {
      if (desktopFile) {
        ...
        await supabaseAdmin.storage.from('hero-media').upload(path, processed, { contentType: 'image/webp' });
      ...
      if (videoFile) {
        ...
        await supabaseAdmin.storage.from('hero-media').upload(path, Buffer.from(buffer), { contentType: videoFile.type });
        desktopUrl = supabaseAdmin.storage.from('hero-media').getPublicUrl(path).data.publicUrl;
      }
    }
