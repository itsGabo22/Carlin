...
export function CategoriasClient({ categories }: { categories: CategoryRow[] }) {
  ...
  const openNewModal = () => {
    setEditingId(null);
    setForm({ name: '', slug: '', parentId: '', groupByBrand: false });
    setModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.error || 'Error guardando la categoría');
        setSaving(false);
      }
    } catch (error) {
      console.error(error);
  ...
  const handleDelete = async (category: CategoryRow) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        alert(data.error || 'Error eliminando la categoría');
      }
    } catch (error) {
      console.error(error);
      alert('Error de conexión');
  ...
  const renderActions = (category: CategoryRow, deleteDisabled: boolean) => (
              ...
              return (
                <div key={category.id} className="flex flex-col">
                  ...
                  <div className="grid grid-cols-12 items-center px-4 py-3 hover:bg-gray-50 transition-colors">
                    ...
                    {renderActions(category, category._count.products > 0 || category._count.children > 0)}
                  </div>
                  ...
                  {children.map(child => (
                    <div key={child.id} className="grid grid-cols-12 items-center px-4 py-2 hover:bg-gray-50 transition-colors border-t border-gray-50 bg-gray-50/30">
                      ...
                      {renderActions(child, child._count.products > 0)}
                    </div>
                  ))}
                </div>
              );
